...
export default function ImageProductAttributes({
  handleUploadImages,
  onDelete,
  idUpload,
  images,
}) {
  const [downloadURLs, setDownloadURLs] = useState(images || []);
  ...
  const handleFileUpload = useCallback(
    (e) => {
      ...
      files.forEach((file) => {
        ...
        const storageRef = ref(imageDb, `products/attributes/${uuidv4()}`);
          ...
          async () => {
            try {
              ...
              setDownloadURLs((prevURLs) => {
                ...
                // Pass updated URLs to the parent component
                handleUploadImages(updatedURLs);
                return updatedURLs;
              });
  ...
  const handleDelete = useCallback(
    ...
    [imageRefs, downloadURLs, onDelete]
  );
  ...
  return (
    <Box textAlign="center" p={3}>
      <input
        type="file"
        id={`uploadImage${idUpload}`}
        accept="image/*"
        style={{ display: "none" }}
        onChange={handleFileUpload}
        multiple
      />
      <label htmlFor={`uploadImage${idUpload}`}>
        <Box
          sx={{
            width: 110,
            height: 110,
