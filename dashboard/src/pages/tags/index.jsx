...
import {
  ...
  getAllTags,
  updateTag,
} from "../../redux/slices/tags"; // Added updateTag action
...
export default function TagPage() {
  ...
  const validationSchema = Yup.object({
    name: Yup.string()
      ...
      .matches(
        /^[a-zA-Z0-9!@#\$%\^\&*\)\(+=._-]+$/,
        "Tên tag không được chứa khoảng trống hoặc dấu"
      ),
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      if (editTag) {
        dispatch(updateTag({ tagId: editTag._id, data: { ...values } })).then(
          (res) => {
  ...
  return (
    <div
      style={{ display: "grid", gridTemplateColumns: "7fr 5fr", gap: "20px" }}
    >
      <div>
        {data && (
          <ReusableTable
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <ImageUploader
                  ...
                  folder="brand"
                  dataImage={formik.values.image ? [formik.values.image] : []}
                />
              </Grid>
              <Grid item xs={12}>
                <CustomInputField
